...
def BFS(s):
    t_s = str(sorted(s[1]))+str(sorted(s[2]))
    level = set([t_s])
    parent = {t_s: [None, None]}
    i = 1
    frontier = [s]
    max_= 0
    while i <= 3:
        ...
        for u in frontier:
            ...
            for r1, c1 in ar:
                for r, c in step:
                    r2, c2 = [r1 + r, c1 + c]
                    arr = copy.deepcopy(u)
                    v = move(arr, r1, c1, r2, c2, idx)
                    t_v = str(sorted(v[1])) + str(sorted(v[2]))
                    if t_v not in level:
                        level.add(t_v)
                        parent[t_v] = [t_s, [r1, c1, r2, c2]]
                        if i < 3:nexti.append(v)
                        elif len(v[1]) > max_:
                            max_ = len(v[1])
                            pos = t_v
        frontier = nexti
